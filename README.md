...
{When you focus in on a video that was muted by default the volume will turn on which is not the intended behavior could have been caught with Unit testing}
...
{UI error that had a pop up keyboard covering the account selection for changing an accountcould have been caught with Visual Regresstion testing}
...
3. There should be a standard that requires unit testing for all changes made to the codebase and integration testing for large modual changes and additions to            functionality.
